...
func errorValidationHandler[T any](err error, payload *T) (int, map[string][]string) {
	...
	for _, err := range err.(validator.ValidationErrors) {
		...
		if err.Param() != "" {
			...
			message = fmt.Sprintf("validation for '%s' failed on the '%s' with parameter '%s'", fieldInMsg, err.Tag(), err.Param())
		} else {
			// message = fmt.Sprintf("field validation for '%s' failed on the '%s' tag", field, err.Tag())
			message = fmt.Sprintf("validation for '%s' failed on the '%s'", fieldInMsg, err.Tag())
		}
		...
		case "required":
			...
			message = fmt.Sprintf("%s required.", fieldInMsg)
		case "email":
			// message = fmt.Sprintf("%s is not a valid email address.", field)
			message = fmt.Sprintf("%s not a valid email address.", fieldInMsg)
		...
		case "strong_password":
			...
			message = fmt.Sprintf("%s It is at least 12 characters and must contain at least one uppercase letter, one lowercase letter, and one number.", fieldInMsg)
		case "exist":
