...
type Config struct {
	...
	HackoStorage struct {
		...
		Secret   string `env:"HACKO_STORAGE_SECRET"`
		Endpoint string `env:"HACKO_STORAGE_ENDPOINT"`
		Region   string `env:"HACKO_STORAGE_REGION"`
		Bucket   string `env:"HACKO_STORAGE_BUCKET"`
	}
	Oauth struct {
		Google struct {
