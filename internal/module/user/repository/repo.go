...
func (r *userRepository) Register(ctx context.Context, req *entity.RegisterRequest) (*entity.RegisterResponse, error) {
	...
	if err != nil {
		...
		if !ok {
			...
			return nil, errmsg.NewCustomErrors(500, errmsg.WithMessage("Failed to insert data"))
		}
...
`
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			return nil, errmsg.NewCustomErrors(401, errmsg.WithMessage("User not found"))
		}
...
func (r *userRepository) UpdateRefreshToken(ctx context.Context, userId, refreshToken string) error {
	...
	if err != nil {
		...
		return errmsg.NewCustomErrors(500, errmsg.WithMessage("Failed to save refresh token"))
	}

	return nil
}

func (r *userRepository) FindRefreshToken(ctx context.Context, refreshToken string) (*entity.UserPayload, error) {
	var userPayload = new(entity.UserPayload)
	...
	err := r.db.GetContext(ctx, userPayload, r.db.Rebind(query), refreshToken)
	if err != nil {
		if err == sql.ErrNoRows {
			log.Warn().Err(err).Str("refreshToken", refreshToken).Msg("repo::FindRefreshToken - Refresh token not found")
			return nil, errmsg.NewCustomErrors(400, errmsg.WithMessage("Refresh token not found"))
		}

		log.Error().Err(err).Str("refreshToken", refreshToken).Msg("repo::FindRefreshToken - Failed to retrieve refresh token")
		return nil, errmsg.NewCustomErrors(500, errmsg.WithMessage("Failed to retrieve refresh token"))
	}
